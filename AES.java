...
Successfully!!!",Toast.LENGTH_SHORT).show();
 ...
 inptext = input_text.getText().toString();
 // pwdtext = password_text.getText().toString();
 outputstring = decrypt(inptext, pwdtext);// outputstring was there in place of 
inptext
 output_text.setText(outputstring);
 //make a toast her e to say deccypted successfully
 // Toast.makeText(this,"decrypted 
Successfully!!!",Toast.LENGTH_SHORT).show();
 } catch (Exception e) {
 e.printStackTrace();
 }
...
operation
 ...
 //It's basically a way of encoding arbitrary binary data in ASCII text. It takes 4 characters 
per 3 bytes of data,
 // plus potentially a bit of padding at the end.
 //Essentially each 6 bits of the input is encoded in a 64-character alphabet.
 //The "standard" alphabet uses A-Z, a-z, 0-9 and + and /, with = as a padding character. 
There are URL-safe variants.
 ...
 byte[] decodedvalue = Base64.decode(data, Base64.DEFAULT);//pehle vo base64 me 
encoded tha, to decode to karna padega na
 ...
 final MessageDigest digest = MessageDigest.getInstance("SHA-256");//for using hash 
function SHA-256
 byte[] bytes = password.getBytes("UTF-8");
 digest.update(bytes, 0, bytes.length);//process kr bytes array ko
 byte[] key = digest.digest();////Completes the hash computation by performing final 
...
intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL,RecognizerIntent.LANG
UAGE_MODEL_FREE_FORM);
 ...
 {
 Toast.makeText(this,"your device does not support this 
